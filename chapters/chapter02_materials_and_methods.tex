...
To obtain this result, real signal and noise components must be separated and the synthetic matrices will be the sum the real signal component and a randomly perturbed noise one. 
...
\begin{itemize}
    ...
    \end{equation}
    where $d$ is the damping factor, $M_i$ are nodes linking to $i$ and $k_i^{out}$ is the out-degree of node $j$ (\cite{page1999pagerank}). High PageRank nodes are influential, considering both quantity and quality of connections. 
\end{itemize}
