...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{chapters/images/Chapter 03/BIL_chr1_list_1copy.npy.png}
...
\begin{table}[H]
    ...
    \hline
    Degree Count & 0.670 & 0.0 & $1.773\times 10^{-39}$ \\ 
    \hline
    Strength & 0.133 & 0.0 & 1.000\\ 
    \hline
    Betweenness & 0.329 & 0.0 & 0.136 \\ 
    \hline
    Closeness & 0.186 & 0.0 & 1.000 \\ 
    \hline
    Eigenvector & 0.122 & 0.0 & 1.000 \\ 
    \hline
    Clustering (weighted) & 0.182 & 0.0 & 0.999 \\ 
    \hline
    PageRank  & 0.138 & 0.0 & 1.000 \\
    \hline
    \end{tabular}
...
For that puropose, Kolmogorov-Smirnov (KS) test has been performed for every centrality measure. As shown in the fourth column of the Table \ref{tab: validation of sy networks}, the associated p-value is $>0.05$ for all the metrics except for the \textit{Degree Count}, which means that there is no significant difference between real and synthetic data. 
For the \textit{Degree Count} metric, the Kolmogorov–Smirnov test detects a global difference in the distribution; however, this effect is most likely attributable to the high sensitivity of the KS test when applied to discrete variables, especially in the presence of large sample sizes, rather than to genuine structural discrepancies. Notably, no significant local deviations are observed for this metric.

\begin{figure}[H]
    \centering
    \begin{minipage}[t]{.4\linewidth}
        \centering
        \includegraphics[height=9cm]{chapters/images/Chapter 03/betweenness_comparison.png}
    \end{minipage}
    \hspace{0.1\linewidth}
    \begin{minipage}[t]{.4\linewidth}
        \centering
        \includegraphics[height=9cm]{chapters/images/Chapter 03/degree_count_comparison.png}
    \end{minipage}
    \caption{Visual representation of the superposition of real and synthetic values, Z-scores, and FDR p-values for two selected metrics: Betweenness (left) and Degree Count (right) for the (\textit{BIL\_chr1\_list}) sample.}
    \label{fig: bet and DG comparison}
\end{figure}

To support the validity of these results, we can examine the graphs reported in Figure \ref{fig: bet and DG comparison}, which provide a visual representation of the superposition of real and synthetic values, Z-scores, and FDR p-values for two selected metrics: Betweenness and Degree Count. Consistent with the previous table (Table \ref{tab: validation of sy networks}), Betweenness shows a precise superposition, while Degree Count is slightly less exact but still acceptable: most of the real data remain within the standard deviation of the synthetic copies, the Z-score lies within the $\pm2$ range for most of the sample copies, and the FDR p-value $>0.05$.
This suggests that the KS result for Degree Count is driven by the test's global sensitivity, not by any true structural discrepancy.
Overall, the synthetic networks faithfully preserve both the local and global structural properties of the original Hi-C networks.
...
\\
In the previous visualization one case was taken as representative example, which doesn't allow a full assessment of the variability across different cell types. To address this, all six cell types were projected into a common PCA space, using the mean centrality vector of each dataset.
Real datasets are represented as blue points, while synthetic datasets are shown as orange points in the Figure \ref{fig: global pca}. 
\begin{figure}[H]
    \centering
    \includegraphics[height= 5.5cm]{}
    \caption{PCA projection of all six cell types. Blue points represent real datasets, while orange points correspond to the synthetic datasets. Each point shows the mean centrality vector of a dataset.}
    \label{fig: global pca}
\end{figure}


This visualization allows a direct comparison between real and synthetic data across all cell types, showing how well the synthetic data preserves the structure of the original data. 
Overlaps between blue and orange points indicate that the synthetic datasets closely replicate the distribution of the original data, whereas deviations highlight potential discrepancies in capturing the full variability of each cell type.